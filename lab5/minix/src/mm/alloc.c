...
/* Zad5 */
#define FIRST_FIT_ALG_VALUE 0
#define WORST_FIT_ALG_VALUE 1
...
PUBLIC int do_worst_fit(void) 
{
  ...
  printf("do_worst_fit\n");
  mm_worst_fit(mm_in.m1_i1);
  printf("selected_fit_algorithm: %d\n", selected_fit_algorithm);
  return 0;
}
...
PUBLIC int do_hole_map(void) 
{
  ...
  while (hp != NIL_HOLE && hp->h_base < swap_base && i < nbytes - 2) {
    buffer[i] = hp->h_len;
...
int w; 
{
  ...
  if (w == 0) 
  {
    selected_fit_algorithm = FIRST_FIT_ALG_VALUE;
  }
  else if (w == 1) 
  {
    selected_fit_algorithm = WORST_FIT_ALG_VALUE;
  }
}
...
phys_clicks clicks;        /* amount of memory requested */
{
  ...
  if (selected_fit_algorithm == WORST_FIT_ALG_VALUE) {
    return mm_alloc_worst_fit(clicks);
  }
  else {
    return mm_alloc_first_fit(clicks);
  }
}


PRIVATE phys_clicks mm_alloc_worst_fit(clicks)
phys_clicks clicks;
{
  /* Zad5 */

  register struct hole *hp, *prev_ptr;
  register struct hole *biggest_hp;

  phys_clicks old_base;
  phys_clicks biggest_clicks;
  ...
  do {
    ...
    /* Find biggest free hole */
    while (hp != NIL_HOLE && hp->h_base < swap_base) {
      if (hp->h_len >= clicks && hp->h_len > biggest_clicks) {
        biggest_hp = hp;
        biggest_clicks = hp->h_len;
      }
      prev_ptr = hp;
      hp = hp->h_next;
    }

    if (biggest_hp != NIL_HOLE) {
      old_base = biggest_hp->h_base;
      biggest_hp->h_base += clicks;
      biggest_hp->h_len -= clicks;

      if (biggest_hp->h_len == 0) del_slot(prev_ptr, biggest_hp);
...
PRIVATE phys_clicks mm_alloc_first_fit(clicks)
phys_clicks clicks;
{
