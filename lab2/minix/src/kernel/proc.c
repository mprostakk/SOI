 ...
 *				sched					     * 
 *===========================================================================*/
PRIVATE struct proc* findProcGroup(int group) {
  struct proc *p;
  p = rdy_head[USER_Q];
  while (p != NIL_PROC) {
    if (p->group == group) {
...
PRIVATE void moveToStart(p)
register struct proc* p;
{
  register struct proc *prev, *next;
  if (rdy_head[USER_Q] == p) {
    return;
  }

  for (prev = rdy_head[USER_Q]; prev->p_nextready != p; prev = prev->p_nextready);
  next = p->p_nextready;
...
int selected_group = GROUP_A;
int count = GROUP_A_SCHED_TICKS_COUNT;
 ...
 */
  ...
  p = findProcGroup(selected_group);
  ...
  if (rdy_head[USER_Q]->group == GROUP_A) {
    count = GROUP_A_SCHED_TICKS_COUNT;
  }
  else {
    count = GROUP_B_SCHED_TICKS_COUNT;
  }
